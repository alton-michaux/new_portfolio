import React from "react";
import Nav from "../Nav";
import Footer from "../Footer";
import PropTypes from "prop-types";
import styles from "../../css/About.module.css"

const About = () => {
  return (
    <>
      <Nav></Nav>
      <div className="about-me-page-greet-box container-fluid">
        <div className="py-3 container-fluid about-me-text-box">
          <p className="greet">As a back end developer, I'm knowledgable in Object Oriented Programing: specifically Ruby on Rails techonologies.
            <br> This has allowed me to gain experience with MVC concepts, ORM and RESTful APIs.
            </br>
            <br> I also have experience in front end development with HTML, CSS, and JavaScript.
            </br>
            <br> As a self-motivated individual who is always looking to learn new skills and improve my current ones, it gives me a sense of pride to be able to provide efficient business solutions to my clients/employers.
            </br>
          </p>
        </div>
        <div className="language-img-container mt-1 d-inline-flex">
          <img
            src="https://www.iglobsyn.com/wp-content/uploads/2019/09/Ruby-on-rails-development.png"
            alt="ruby on rails"
            className="thumbnail"
            style="height: 5em;" />
          <img
            src="https://images.vexels.com/media/users/3/166382/isolated/preview/1ad81b62ad0ec81a584bc22016fd016f-html-programming-language-flat.png"
            alt="html5"
            className="thumbnail"
            style="height: 5em;" />
          <img
            src="https://www.coherentsolutions.com/wp-content/uploads/2020/08/css-logo-white.png"
            alt="css"
            className="thumbnail"
            style="height: 5em;" />
          <img
            src="data:image/png;base64,"
            alt="javascript"
            className="thumbnail"
            style="height: 5em;" />
        </div>
      </div>
      <Footer></Footer>
    </>
  )
}

export default About
